...
char
	*ip1_name = NULL,
	*ip2_name = NULL,
	*echo_opts[] = {"Don't Care", "Backspace", "BSSlash", "BsSpBs"},
	*echo_cntl[] = {"Don't Care", "Don't Allow", "SwitchOk", "NotifyOnSwitch"};
...
{/*FormatEnvCntl*/
	...
	*ptr;
	...
	printf("  echo_resp=$%02x[%s], echo_cntl=$%02x[%s]\n",
	       am_req->fEcho, echo_opts[am_req->fEcho],
	       am_req->fEchoControl, echo_cntl[am_req->fEchoControl]);
...
{/*FormatTerminalCntl*/
	...
	*driver_resp = (tVTMTerminalDriverControlResponse*)vt;
	...
	if (mask & kTDCMEcho)
	    printf(" echo_cntl=$%02x[%s]", driver_req->fEcho,
		   echo_cntl[driver_req->fEcho]);
